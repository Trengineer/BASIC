...
from ds import prepare_coco_dataloaders, prepare_flickr_dataloaders, prepare_cub_dataloaders, prepare_flo_dataloaders, prepare_cub_dataloaders_extra, prepare_foodredmeat_dataloaders, prepare_foodmeat_dataloaders
...
# New argument for redOrNot as a boolean flag
#parser.add_argument('--red', action='store_true', help=help="Set this flag if you want redmeat dataset, otherwise don't use it at all")
...
checkpoint_path = '../ProbVLM/ckpt/ProbVLM_FOOD_Meat_17.02_best.pth'

dataset = 'food-101'  
filename = '../ProbVLM/Datasets/food-101/data_loaders_food_meat.pkl'
att_folder_mean = 'clip_rn50_attention_gradcam_food_meat_mean'
att_folder_median = 'clip_rn50_attention_gradcam_food_meat_median'
...
def load_data_loader(dataset, data_dir, dataloader_config):
    prepare_loaders = {
        ...
        'waterbird_complete95_forest2water2': prepare_cub_dataloaders_extra,
        'FLO':prepare_flo_dataloaders,
        'food-101': prepare_foodmeat_dataloaders,
        'food-redmeat': prepare_foodredmeat_dataloaders
    }[dataset]
    if dataset == 'CUB' or dataset == 'waterbird_1.0_forest2water2' or dataset == 'waterbird_complete95_forest2water2':
        ...
        train_loader, val_loader, test_loader = prepare_loaders(transform=transform,root=data_dir, cfg ='..GALS/configs/base.yaml', seed=42)
        return train_loader, val_loader, test_loader
    elif dataset == 'food-redmeat':
        transform = transforms.Compose([
            transforms.Resize((224, 224)),  # Resize all images to 224x224
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
...
data_dir = ospj('../ds-project/ProbVLM/Datasets/', dataset)
...
text_list = ["an image of an animal based meat", "a photo of an animal based meat"]
...
def process_attention_maps(data_loader, save_folder, aggregation_method, max_files=15):
    ...
    ROOT = "../ds-project/GALS/data/food-101"
    ...
    for i, batch in enumerate(data_loader):
        xI, xT, paths = batch['image'].to(device), batch['tokenized label'].to(device), batch['image_path']
        
        for t, (img, txt, path) in enumerate(zip(xI, xT, paths)):
            text_list = token_to_text(xT[t])

            # Filter by text content
            if text_list[0] not in {'a photo of animal based meat', 'an image of animal based meat'}:
                continue
...
process_attention_maps(cub_train_loader, att_folder_mean, "mean")
process_attention_maps(cub_valid_loader, att_folder_mean, "mean")
process_attention_maps(cub_train_loader, att_folder_median , "median")
process_attention_maps(cub_valid_loader, att_folder_median, "median")
